...
// Each producer is making 3 slots of items and we have 2 producer so we have total 6 slots of items that is producing the producer
//then we decrement by sem_wait(&empty) and showing that one slot is taken form the buffer

//then we will secure the critical section by mutex lock and unlock
//sem_post(&full) Signal that buffer has a full slot

// Each consumer is consuming 3 slots of items and we have 2 consumers so we have total 6 slots of items that is consuming the consumer
//then we decrement by sem_wait(&full) and showing that one item is taken from the buffer
//then we will secure the critical section by mutex lock and unlock
